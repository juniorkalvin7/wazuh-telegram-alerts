#!/usr/bin/env python3
# wazuh-telegram-openai.py - Resumo com análise IA usando OpenAI API
...
logging.basicConfig(
    ...
    datefmt='%Y-%m-%d %H:%M:%S',
    handlers=[
        logging.FileHandler('/var/log/wazuh-telegram-openai.log'),
...
if not OPENAI_API_KEY:
    logger.error("OPENAI_API_KEY não definida! Configure com sua chave OpenAI.")
    exit(1)
...
async def analyze_with_ia(events, rid, desc, count, level):
    ...
    try:
        r = requests.post(
            ...
            json={
                "model": "gpt-4o-mini",  # Rápido e econômico (pode trocar por gpt-4o se quiser mais precisão)
        ...
        logger.info(f"Análise IA (OpenAI) para rule {rid}: {analysis[:100]}...")
        return analysis
    except requests.exceptions.HTTPError as e:
        logger.error(f"Erro HTTP na OpenAI API (status {r.status_code}): {r.text[:300]}")
        return "Erro na análise IA (verifique chave OpenAI ou limite de uso)."
    except Exception as e:
        logger.error(f"Erro geral na OpenAI API: {e}")
    ...
    while True:
        ...
        for (rid, desc), events in sorted(groups.items(), key=lambda x: len(x[1]), reverse=True):
            ...
            msg = (
                ...
                f"🤖 <b>Análise IA (OpenAI):</b> {ia_analysis}\n\n"
